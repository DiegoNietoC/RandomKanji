...
const updateRandomNumber = () => {
	Minimo = parseInt(document.getElementById('Min_').value,10);
	Maximo = parseInt(document.getElementById('Max_').value,10);

	if(Minimo>Maximo || Minimo<1 || Maximo>500){
		Minimo = 1;
		Maximo = 500;
	}

	let random_num = Math.floor((Math.random()*(Maximo - Minimo + 1)) + Minimo);

	document.getElementById('random-significado').innerHTML	= (significados[random_num]);
	document.getElementById('random-kanji').innerHTML 		= (kanji[random_num]);
	document.getElementById('random-number').innerHTML 		= ("- "+random_num+" -");
}

document.getElementById('botonGenerador').onclick = function() {
		updateRandomNumber();	
	}
